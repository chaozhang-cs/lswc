...
import lswc.baselines.FullyDynamicConnectivity;
import lswc.baselines.utils.InternalGraph;
import lswc.baselines.utils.InternalVertex;
    ...
    public boolean connected(int source, int target) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (visited.equals(internalTarget))
                return true;
            for (InternalVertex vertex : visited.getAdjacencyList()) {
                if (vSet.add(vertex))
                    stack.push(vertex);
            }
        }
