...
public class WorkloadUtils {
    ...
    public static List<IntIntPair> readWorkload(String path) {
    ...
    private static void addQueries(UnionFindTree incrementalConnectivity, List<IntIntPair> ret, List<IntIntPair> vertexAndDegree, Random random, Graph<Integer, DefaultEdge> graph, int num, int start, int bound1, int bound2) {
        int i = 0;
        while (i++ < num) {
            int s = vertexAndDegree.get(random.nextInt(bound1) + start).firstInt();
            int t = vertexAndDegree.get(random.nextInt(bound2) + start).firstInt();
